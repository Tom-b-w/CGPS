...
def get_arguments():
    ...
    parser.add_argument('--prompt-mode', dest='prompt_mode', type=str, default='standard', choices=['standard', 'cupl', 'both'], help='Prompt source: hand-crafted templates, CuPL prompts, or both.')
    parser.add_argument('--cupl-path', dest='cupl_path', type=str, default=None, help='Path to CuPL prompt JSON. Supports {dataset} placeholder.')
...
def main():
    ...
    for dataset_name in datasets:
        ...
        cupl_path = args.cupl_path.format(dataset=dataset_name) if args.cupl_path else None
        logger.info(f"Prompt mode: {args.prompt_mode}")
        logger.info(f"CuPL path: {cupl_path}")
        clip_weights = clip_classifier(classnames, template, clip_model, prompt_mode=args.prompt_mode, cupl_path=cupl_path)
