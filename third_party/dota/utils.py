...
def load_cupl_prompts(cupl_path):
    if cupl_path is None:
        return None
...
def build_prompt_texts(classname, template, prompt_mode='standard', cupl_prompts=None):
    if prompt_mode not in {'standard', 'cupl', 'both'}:
        raise ValueError(f"Unknown prompt mode: {prompt_mode}")

    raw_classname = classname
    classname = classname.replace('_', ' ')
    handcrafted_texts = [t.format(classname) for t in template]
    if prompt_mode == 'standard':
        return handcrafted_texts

    if cupl_prompts is None:
        raise ValueError("CuPL prompts are required when prompt_mode is 'cupl' or 'both'.")

    candidate_keys = [classname, raw_classname, raw_classname.replace('_', ' ')]
    cupl_texts = None
    for key in candidate_keys:
        if key in cupl_prompts:
            cupl_texts = list(cupl_prompts[key])
            break

    if cupl_texts is None:
        raise KeyError(f"missing CuPL prompts for class '{classname}'")

    if prompt_mode == 'cupl':
        return cupl_texts

    return handcrafted_texts + cupl_texts


def clip_classifier(classnames, template, clip_model, prompt_mode='standard', cupl_path=None):
    cupl_prompts = load_cupl_prompts(cupl_path)
    ...
    with torch.no_grad():
        ...
        for classname in classnames:
            ...
            texts = build_prompt_texts(classname, template, prompt_mode, cupl_prompts)
...
def build_test_data_loader(dataset_name, root_path, preprocess):
    if dataset_name == 'I':
        ...
        test_loader = torch.utils.data.DataLoader(dataset.test, batch_size=1, num_workers=2, shuffle=True)
